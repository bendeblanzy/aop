...
import { createCollaborateurSchema, updateCollaborateurSchema, deleteByIdSchema } from '@/lib/validations'
...
export async function POST(request: NextRequest) {
  ...
  const parsed = await parseBody(request, createCollaborateurSchema)
...
export async function PUT(request: NextRequest) {
  ...
  const parsed = await parseBody(request, updateCollaborateurSchema)
  if (parsed.error) return parsed.error
