  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-text-primary">Appels d&apos;offres</h1>
          <p className="text-text-secondary mt-1">{aos?.length ?? 0} appel{(aos?.length ?? 0) > 1 ? 's' : ''} d&apos;offres</p>
        </div>
        <Link
            ...
            <div key={ao.id} className="flex items-center justify-between px-6 py-5 hover:bg-surface transition-colors group">
              <Link href={`/appels-offres/${ao.id}`} className="flex-1 min-w-0 mr-4">
                ...
                <div className="flex items-center gap-4 mt-1">
                  {ao.acheteur && <span className="text-sm text-text-secondary">{ao.acheteur}</span>}
                  {ao.reference_marche && <span className="text-sm text-text-secondary">Réf. {ao.reference_marche}</span>}
